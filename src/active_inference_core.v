/* Generated by Amaranth Yosys 0.50 (PyPI ver 0.50.0.0.post126, git sha1 b5170e139) */

module active_inference_core(tick, bsel, gamma, csel, clk, rst, action, ready, belief_dbg, obs);
  reg \$auto$verilog_backend.cc:2355:dump_module$1  = 0;
  wire \$1 ;
  wire [8:0] \$10 ;
  wire \$100 ;
  wire \$101 ;
  wire \$102 ;
  wire \$103 ;
  wire \$104 ;
  wire \$105 ;
  wire \$106 ;
  wire \$107 ;
  reg \$108 ;
  reg [1:0] \$109 ;
  wire \$11 ;
  reg [1:0] \$110 ;
  reg \$111 ;
  reg \$112 ;
  reg [2:0] \$113 ;
  reg [7:0] \$114 ;
  reg [7:0] \$115 ;
  reg [7:0] \$116 ;
  reg [9:0] \$117 ;
  reg [9:0] \$118 ;
  reg [9:0] \$119 ;
  wire [8:0] \$12 ;
  reg [14:0] \$120 ;
  reg [14:0] \$121 ;
  reg [14:0] \$122 ;
  reg [1:0] \$123 ;
  wire \$13 ;
  wire [8:0] \$14 ;
  wire [8:0] \$15 ;
  wire \$16 ;
  wire [8:0] \$17 ;
  wire \$18 ;
  wire [8:0] \$19 ;
  wire \$2 ;
  wire \$20 ;
  wire \$21 ;
  wire [8:0] \$22 ;
  wire \$23 ;
  wire [8:0] \$24 ;
  wire \$25 ;
  wire [8:0] \$26 ;
  wire [8:0] \$27 ;
  wire \$28 ;
  wire [8:0] \$29 ;
  wire [6:0] \$3 ;
  wire \$30 ;
  wire [8:0] \$31 ;
  wire [8:0] \$32 ;
  wire \$33 ;
  wire [8:0] \$34 ;
  wire \$35 ;
  wire [8:0] \$36 ;
  wire [8:0] \$37 ;
  wire \$38 ;
  wire [8:0] \$39 ;
  wire \$4 ;
  wire \$40 ;
  wire [8:0] \$41 ;
  wire [8:0] \$42 ;
  wire \$43 ;
  wire [8:0] \$44 ;
  wire \$45 ;
  wire [8:0] \$46 ;
  wire [8:0] \$47 ;
  wire \$48 ;
  wire [8:0] \$49 ;
  wire [8:0] \$5 ;
  wire \$50 ;
  wire [8:0] \$51 ;
  wire [8:0] \$52 ;
  wire [8:0] \$53 ;
  wire \$54 ;
  wire [8:0] \$55 ;
  wire \$56 ;
  wire [8:0] \$57 ;
  wire [8:0] \$58 ;
  wire \$59 ;
  wire \$6 ;
  wire [8:0] \$60 ;
  wire \$61 ;
  wire [8:0] \$62 ;
  wire [8:0] \$63 ;
  wire \$64 ;
  wire [8:0] \$65 ;
  wire \$66 ;
  wire [8:0] \$67 ;
  wire [8:0] \$68 ;
  wire [8:0] \$69 ;
  wire [8:0] \$7 ;
  wire \$70 ;
  wire [8:0] \$71 ;
  wire \$72 ;
  wire [8:0] \$73 ;
  wire [8:0] \$74 ;
  wire \$75 ;
  wire [8:0] \$76 ;
  wire \$77 ;
  wire [8:0] \$78 ;
  wire [8:0] \$79 ;
  wire \$8 ;
  wire \$80 ;
  wire [8:0] \$81 ;
  wire \$82 ;
  wire [8:0] \$83 ;
  wire [8:0] \$84 ;
  wire \$85 ;
  wire \$86 ;
  wire [10:0] \$87 ;
  wire [13:0] \$88 ;
  wire [14:0] \$89 ;
  wire [8:0] \$9 ;
  wire [15:0] \$90 ;
  wire [10:0] \$91 ;
  wire [13:0] \$92 ;
  wire [14:0] \$93 ;
  wire [15:0] \$94 ;
  wire [10:0] \$95 ;
  wire [13:0] \$96 ;
  wire [14:0] \$97 ;
  wire [15:0] \$98 ;
  wire \$99 ;
  wire [7:0] C0;
  wire [7:0] C1;
  wire [7:0] C2;
  output [1:0] action;
  reg [1:0] action = 2'h0;
  reg [7:0] belief0 = 8'h00;
  reg [7:0] belief1 = 8'h00;
  reg [7:0] belief2 = 8'h00;
  output [7:0] belief_dbg;
  reg [7:0] belief_dbg;
  reg [14:0] best01;
  input [1:0] bsel;
  wire [1:0] bsel;
  reg [1:0] ch01;
  reg [1:0] chosen;
  input clk;
  wire clk;
  input csel;
  wire csel;
  reg csel_r = 1'h0;
  wire [11:0] d_0;
  wire [11:0] d_1;
  wire [11:0] d_2;
  reg [7:0] ev0;
  reg [7:0] ev1;
  reg [7:0] ev2;
  reg [2:0] fsm_state = 3'h0;
  input [1:0] gamma;
  wire [1:0] gamma;
  reg [1:0] gamma_r = 2'h0;
  reg [7:0] mx01;
  reg [7:0] mxall;
  reg [7:0] nb0;
  reg [7:0] nb1;
  reg [7:0] nb2;
  input [1:0] obs;
  wire [1:0] obs;
  reg [1:0] obs_r = 2'h0;
  output ready;
  reg ready = 1'h0;
  input rst;
  wire rst;
  wire [9:0] sc_0;
  wire [9:0] sc_1;
  wire [9:0] sc_2;
  reg [9:0] sc_r0 = 10'h000;
  reg [9:0] sc_r1 = 10'h000;
  reg [9:0] sc_r2 = 10'h000;
  reg [14:0] sh_0;
  reg [14:0] sh_1;
  reg [14:0] sh_2;
  reg [14:0] sharp_r0 = 15'h0000;
  reg [14:0] sharp_r1 = 15'h0000;
  reg [14:0] sharp_r2 = 15'h0000;
  reg [9:0] smin;
  reg [9:0] smin01;
  reg [7:0] t_0_0;
  reg [7:0] t_0_1;
  reg [7:0] t_0_2;
  reg [7:0] t_1_0;
  reg [7:0] t_1_1;
  reg [7:0] t_1_2;
  reg [7:0] t_2_0;
  reg [7:0] t_2_1;
  reg [7:0] t_2_2;
  input tick;
  wire tick;
  reg tick_d = 1'h0;
  reg [7:0] upd0;
  reg [7:0] upd1;
  reg [7:0] upd2;
  assign \$1  = ! csel_r;
  assign C0 = csel_r ? 8'hf5 : 8'hb6;
  assign \$2  = ! csel_r;
  assign \$4  = ! csel_r;
  assign C2 = csel_r ? 8'hb6 : 8'hf5;
  assign \$5  = $signed(belief0) + $signed(ev0);
  assign \$6  = $signed(\$5 ) > $signed(8'h7f);
  assign \$7  = $signed(belief0) + $signed(ev0);
  assign \$8  = $signed(\$7 ) < $signed(8'h80);
  assign \$9  = $signed(belief0) + $signed(ev0);
  assign \$10  = $signed(belief1) + $signed(ev1);
  assign \$11  = $signed(\$10 ) > $signed(8'h7f);
  assign \$12  = $signed(belief1) + $signed(ev1);
  assign \$13  = $signed(\$12 ) < $signed(8'h80);
  assign \$14  = $signed(belief1) + $signed(ev1);
  assign \$15  = $signed(belief2) + $signed(ev2);
  assign \$16  = $signed(\$15 ) > $signed(8'h7f);
  assign \$17  = $signed(belief2) + $signed(ev2);
  assign \$18  = $signed(\$17 ) < $signed(8'h80);
  assign \$19  = $signed(belief2) + $signed(ev2);
  assign \$20  = $signed(upd1) > $signed(upd0);
  assign \$21  = $signed(upd2) > $signed(mx01);
  assign \$22  = $signed(upd0) - $signed(mxall);
  assign \$23  = $signed(\$22 ) > $signed(8'h7f);
  assign \$24  = $signed(upd0) - $signed(mxall);
  assign \$25  = $signed(\$24 ) < $signed(8'h80);
  assign \$26  = $signed(upd0) - $signed(mxall);
  assign \$27  = $signed(upd1) - $signed(mxall);
  assign \$28  = $signed(\$27 ) > $signed(8'h7f);
  assign \$29  = $signed(upd1) - $signed(mxall);
  assign \$30  = $signed(\$29 ) < $signed(8'h80);
  assign \$31  = $signed(upd1) - $signed(mxall);
  assign \$32  = $signed(upd2) - $signed(mxall);
  assign \$33  = $signed(\$32 ) > $signed(8'h7f);
  assign \$34  = $signed(upd2) - $signed(mxall);
  assign \$35  = $signed(\$34 ) < $signed(8'h80);
  assign \$36  = $signed(upd2) - $signed(mxall);
  assign \$37  = $signed(belief0) + $signed(C0);
  assign \$38  = $signed(\$37 ) > $signed(8'h7f);
  assign \$39  = $signed(belief0) + $signed(C0);
  assign \$40  = $signed(\$39 ) < $signed(8'h80);
  assign \$41  = $signed(belief0) + $signed(C0);
  assign \$42  = $signed(belief1) + $signed(C0);
  assign \$43  = $signed(\$42 ) > $signed(8'h7f);
  assign \$44  = $signed(belief1) + $signed(C0);
  assign \$45  = $signed(\$44 ) < $signed(8'h80);
  assign \$46  = $signed(belief1) + $signed(C0);
  assign \$47  = $signed(belief2) + $signed(7'h4c);
  assign \$48  = $signed(\$47 ) > $signed(8'h7f);
  assign \$49  = $signed(belief2) + $signed(7'h4c);
  assign \$50  = $signed(\$49 ) < $signed(8'h80);
  assign \$51  = $signed(belief2) + $signed(7'h4c);
  assign \$52  = $signed(t_0_0) + $signed(t_0_1);
  assign sc_0 = $signed(\$52 ) + $signed(t_0_2);
  assign \$53  = $signed(belief0) + $signed(C0);
  assign \$54  = $signed(\$53 ) > $signed(8'h7f);
  assign \$55  = $signed(belief0) + $signed(C0);
  assign \$56  = $signed(\$55 ) < $signed(8'h80);
  assign \$57  = $signed(belief0) + $signed(C0);
  assign \$58  = $signed(belief1) + $signed(7'h4c);
  assign \$59  = $signed(\$58 ) > $signed(8'h7f);
  assign \$60  = $signed(belief1) + $signed(7'h4c);
  assign \$61  = $signed(\$60 ) < $signed(8'h80);
  assign \$62  = $signed(belief1) + $signed(7'h4c);
  assign \$63  = $signed(belief2) + $signed(C2);
  assign \$64  = $signed(\$63 ) > $signed(8'h7f);
  assign \$65  = $signed(belief2) + $signed(C2);
  assign \$66  = $signed(\$65 ) < $signed(8'h80);
  assign \$67  = $signed(belief2) + $signed(C2);
  assign \$68  = $signed(t_1_0) + $signed(t_1_1);
  assign sc_1 = $signed(\$68 ) + $signed(t_1_2);
  assign \$69  = $signed(belief0) + $signed(7'h4c);
  assign \$70  = $signed(\$69 ) > $signed(8'h7f);
  assign \$71  = $signed(belief0) + $signed(7'h4c);
  assign \$72  = $signed(\$71 ) < $signed(8'h80);
  assign \$73  = $signed(belief0) + $signed(7'h4c);
  assign \$74  = $signed(belief1) + $signed(C2);
  assign \$75  = $signed(\$74 ) > $signed(8'h7f);
  assign \$76  = $signed(belief1) + $signed(C2);
  assign \$77  = $signed(\$76 ) < $signed(8'h80);
  assign \$78  = $signed(belief1) + $signed(C2);
  assign \$79  = $signed(belief2) + $signed(C2);
  assign \$80  = $signed(\$79 ) > $signed(8'h7f);
  assign \$81  = $signed(belief2) + $signed(C2);
  assign \$82  = $signed(\$81 ) < $signed(8'h80);
  assign \$83  = $signed(belief2) + $signed(C2);
  assign \$84  = $signed(t_2_0) + $signed(t_2_1);
  assign sc_2 = $signed(\$84 ) + $signed(t_2_2);
  assign \$85  = $signed(sc_r1) < $signed(sc_r0);
  assign \$86  = $signed(sc_r2) < $signed(smin01);
  assign \$87  = $signed(sc_r0) - $signed(smin);
  assign \$88  = { \$87 [10], \$87  } * 2'h2;
  assign \$89  = { \$87 [10], \$87  } * 3'h4;
  assign \$90  = { \$87 [10], \$87  } * 4'h8;
  assign \$91  = $signed(sc_r1) - $signed(smin);
  assign \$92  = { \$91 [10], \$91  } * 2'h2;
  assign \$93  = { \$91 [10], \$91  } * 3'h4;
  assign \$94  = { \$91 [10], \$91  } * 4'h8;
  assign \$95  = $signed(sc_r2) - $signed(smin);
  assign \$96  = { \$95 [10], \$95  } * 2'h2;
  assign \$97  = { \$95 [10], \$95  } * 3'h4;
  assign \$98  = { \$95 [10], \$95  } * 4'h8;
  assign \$99  = sharp_r1 > sharp_r0;
  assign \$100  = sharp_r2 > best01;
  assign \$101  = ! fsm_state;
  assign \$102  = fsm_state == 1'h1;
  assign \$103  = fsm_state == 2'h2;
  assign \$104  = fsm_state == 2'h3;
  assign \$105  = fsm_state == 3'h4;
  assign \$106  = ~ tick_d;
  assign \$107  = tick & \$106 ;
  always @(posedge clk)
    tick_d <= \$108 ;
  always @(posedge clk)
    obs_r <= \$109 ;
  always @(posedge clk)
    gamma_r <= \$110 ;
  always @(posedge clk)
    csel_r <= \$111 ;
  always @(posedge clk)
    ready <= \$112 ;
  always @(posedge clk)
    fsm_state <= \$113 ;
  always @(posedge clk)
    belief0 <= \$114 ;
  always @(posedge clk)
    belief1 <= \$115 ;
  always @(posedge clk)
    belief2 <= \$116 ;
  always @(posedge clk)
    sc_r0 <= \$117 ;
  always @(posedge clk)
    sc_r1 <= \$118 ;
  always @(posedge clk)
    sc_r2 <= \$119 ;
  always @(posedge clk)
    sharp_r0 <= \$120 ;
  always @(posedge clk)
    sharp_r1 <= \$121 ;
  always @(posedge clk)
    sharp_r2 <= \$122 ;
  always @(posedge clk)
    action <= \$123 ;
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    casez (obs_r)
      2'h0:
          ev0 = 8'hf9;
      2'h1:
          ev0 = 8'hb6;
      2'h2:
          ev0 = 8'hb6;
      default:
          ev0 = 8'hf9;
    endcase
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    casez (obs_r)
      2'h0:
          ev1 = 8'hb6;
      2'h1:
          ev1 = 8'hf9;
      2'h2:
          ev1 = 8'hb6;
      default:
          ev1 = 8'hb6;
    endcase
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    casez (obs_r)
      2'h0:
          ev2 = 8'hb6;
      2'h1:
          ev2 = 8'hb6;
      2'h2:
          ev2 = 8'hf9;
      default:
          ev2 = 8'hb6;
    endcase
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$6 ) begin
      upd0 = 8'h7f;
    end else if (\$8 ) begin
      upd0 = 8'h80;
    end else begin
      upd0 = \$9 [7:0];
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$11 ) begin
      upd1 = 8'h7f;
    end else if (\$13 ) begin
      upd1 = 8'h80;
    end else begin
      upd1 = \$14 [7:0];
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$16 ) begin
      upd2 = 8'h7f;
    end else if (\$18 ) begin
      upd2 = 8'h80;
    end else begin
      upd2 = \$19 [7:0];
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$20 ) begin
      mx01 = upd1;
    end else begin
      mx01 = upd0;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$21 ) begin
      mxall = upd2;
    end else begin
      mxall = mx01;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$23 ) begin
      nb0 = 8'h7f;
    end else if (\$25 ) begin
      nb0 = 8'h80;
    end else begin
      nb0 = \$26 [7:0];
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$28 ) begin
      nb1 = 8'h7f;
    end else if (\$30 ) begin
      nb1 = 8'h80;
    end else begin
      nb1 = \$31 [7:0];
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$33 ) begin
      nb2 = 8'h7f;
    end else if (\$35 ) begin
      nb2 = 8'h80;
    end else begin
      nb2 = \$36 [7:0];
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$38 ) begin
      t_0_0 = 8'h7f;
    end else if (\$40 ) begin
      t_0_0 = 8'h80;
    end else begin
      t_0_0 = \$41 [7:0];
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$43 ) begin
      t_0_1 = 8'h7f;
    end else if (\$45 ) begin
      t_0_1 = 8'h80;
    end else begin
      t_0_1 = \$46 [7:0];
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$48 ) begin
      t_0_2 = 8'h7f;
    end else if (\$50 ) begin
      t_0_2 = 8'h80;
    end else begin
      t_0_2 = \$51 [7:0];
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$54 ) begin
      t_1_0 = 8'h7f;
    end else if (\$56 ) begin
      t_1_0 = 8'h80;
    end else begin
      t_1_0 = \$57 [7:0];
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$59 ) begin
      t_1_1 = 8'h7f;
    end else if (\$61 ) begin
      t_1_1 = 8'h80;
    end else begin
      t_1_1 = \$62 [7:0];
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$64 ) begin
      t_1_2 = 8'h7f;
    end else if (\$66 ) begin
      t_1_2 = 8'h80;
    end else begin
      t_1_2 = \$67 [7:0];
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$70 ) begin
      t_2_0 = 8'h7f;
    end else if (\$72 ) begin
      t_2_0 = 8'h80;
    end else begin
      t_2_0 = \$73 [7:0];
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$75 ) begin
      t_2_1 = 8'h7f;
    end else if (\$77 ) begin
      t_2_1 = 8'h80;
    end else begin
      t_2_1 = \$78 [7:0];
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$80 ) begin
      t_2_2 = 8'h7f;
    end else if (\$82 ) begin
      t_2_2 = 8'h80;
    end else begin
      t_2_2 = \$83 [7:0];
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$85 ) begin
      smin01 = sc_r1;
    end else begin
      smin01 = sc_r0;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$86 ) begin
      smin = sc_r2;
    end else begin
      smin = smin01;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    casez (gamma_r)
      2'h0:
          sh_0 = { 3'h0, \$87 [10], \$87  };
      2'h1:
          sh_0 = { 1'h0, \$88  };
      2'h2:
          sh_0 = \$89 ;
      2'h3:
          sh_0 = \$90 [14:0];
    endcase
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    casez (gamma_r)
      2'h0:
          sh_1 = { 3'h0, \$91 [10], \$91  };
      2'h1:
          sh_1 = { 1'h0, \$92  };
      2'h2:
          sh_1 = \$93 ;
      2'h3:
          sh_1 = \$94 [14:0];
    endcase
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    casez (gamma_r)
      2'h0:
          sh_2 = { 3'h0, \$95 [10], \$95  };
      2'h1:
          sh_2 = { 1'h0, \$96  };
      2'h2:
          sh_2 = \$97 ;
      2'h3:
          sh_2 = \$98 [14:0];
    endcase
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$99 ) begin
      ch01 = 2'h1;
    end else begin
      ch01 = 2'h0;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$99 ) begin
      best01 = sharp_r1;
    end else begin
      best01 = sharp_r0;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    if (\$100 ) begin
      chosen = 2'h2;
    end else begin
      chosen = ch01;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    (* full_case = 32'd1 *)
    casez (bsel)
      2'h0:
          belief_dbg = belief0;
      2'h1:
          belief_dbg = belief1;
      2'h2:
          belief_dbg = belief2;
      default:
          belief_dbg = belief0;
    endcase
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    \$108  = tick;
    if (rst) begin
      \$108  = 1'h0;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    \$109  = obs_r;
    casez (fsm_state)
      3'h0:
          if (\$107 ) begin
            \$109  = obs;
          end
    endcase
    if (rst) begin
      \$109  = 2'h0;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    \$110  = gamma_r;
    casez (fsm_state)
      3'h0:
          if (\$107 ) begin
            \$110  = gamma;
          end
    endcase
    if (rst) begin
      \$110  = 2'h0;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    \$111  = csel_r;
    casez (fsm_state)
      3'h0:
          if (\$107 ) begin
            \$111  = csel;
          end
    endcase
    if (rst) begin
      \$111  = 1'h0;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    \$112  = ready;
    casez (fsm_state)
      3'h0:
          if (\$107 ) begin
            \$112  = 1'h0;
          end
      3'h1:
          /* empty */;
      3'h2:
          /* empty */;
      3'h3:
          /* empty */;
      3'h4:
          \$112  = 1'h1;
    endcase
    if (rst) begin
      \$112  = 1'h0;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    \$113  = fsm_state;
    casez (fsm_state)
      3'h0:
          if (\$107 ) begin
            \$113  = 3'h1;
          end
      3'h1:
          \$113  = 3'h2;
      3'h2:
          \$113  = 3'h3;
      3'h3:
          \$113  = 3'h4;
      3'h4:
          \$113  = 3'h0;
    endcase
    if (rst) begin
      \$113  = 3'h0;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    \$114  = belief0;
    casez (fsm_state)
      3'h0:
          /* empty */;
      3'h1:
          \$114  = nb0;
    endcase
    if (rst) begin
      \$114  = 8'h00;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    \$115  = belief1;
    casez (fsm_state)
      3'h0:
          /* empty */;
      3'h1:
          \$115  = nb1;
    endcase
    if (rst) begin
      \$115  = 8'h00;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    \$116  = belief2;
    casez (fsm_state)
      3'h0:
          /* empty */;
      3'h1:
          \$116  = nb2;
    endcase
    if (rst) begin
      \$116  = 8'h00;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    \$117  = sc_r0;
    casez (fsm_state)
      3'h0:
          /* empty */;
      3'h1:
          /* empty */;
      3'h2:
          \$117  = sc_0;
    endcase
    if (rst) begin
      \$117  = 10'h000;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    \$118  = sc_r1;
    casez (fsm_state)
      3'h0:
          /* empty */;
      3'h1:
          /* empty */;
      3'h2:
          \$118  = sc_1;
    endcase
    if (rst) begin
      \$118  = 10'h000;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    \$119  = sc_r2;
    casez (fsm_state)
      3'h0:
          /* empty */;
      3'h1:
          /* empty */;
      3'h2:
          \$119  = sc_2;
    endcase
    if (rst) begin
      \$119  = 10'h000;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    \$120  = sharp_r0;
    casez (fsm_state)
      3'h0:
          /* empty */;
      3'h1:
          /* empty */;
      3'h2:
          /* empty */;
      3'h3:
          \$120  = sh_0;
    endcase
    if (rst) begin
      \$120  = 15'h0000;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    \$121  = sharp_r1;
    casez (fsm_state)
      3'h0:
          /* empty */;
      3'h1:
          /* empty */;
      3'h2:
          /* empty */;
      3'h3:
          \$121  = sh_1;
    endcase
    if (rst) begin
      \$121  = 15'h0000;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    \$122  = sharp_r2;
    casez (fsm_state)
      3'h0:
          /* empty */;
      3'h1:
          /* empty */;
      3'h2:
          /* empty */;
      3'h3:
          \$122  = sh_2;
    endcase
    if (rst) begin
      \$122  = 15'h0000;
    end
  end
  always @* begin
    if (\$auto$verilog_backend.cc:2355:dump_module$1 ) begin end
    \$123  = action;
    casez (fsm_state)
      3'h0:
          /* empty */;
      3'h1:
          /* empty */;
      3'h2:
          /* empty */;
      3'h3:
          /* empty */;
      3'h4:
          \$123  = chosen;
    endcase
    if (rst) begin
      \$123  = 2'h0;
    end
  end
  assign C1 = { \$3 [6], \$3  };
  assign d_0 = { \$87 [10], \$87  };
  assign d_1 = { \$91 [10], \$91  };
  assign d_2 = { \$95 [10], \$95  };
  assign \$3  = 7'h4c;
endmodule
